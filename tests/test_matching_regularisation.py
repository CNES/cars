...
def test_update_disp_0(ref_ds, sec_ds, ref_disp, sec_disp):
    ...
    assert np.allclose(disp[cst.STEREO_REF][cst.DISP_MAP].values,
                       up_ref_disp)
    assert np.allclose(disp[cst.STEREO_REF][cst.DISP_MSK].values,
                       up_ref_disp_msk)
    ...
    sec_msk_set_to_ref_alt = \
        np.logical_or(np.where(sec_ds[cst.EPI_MSK].values == 4, True, False),
        np.where(sec_ds[cst.EPI_MSK].values == 5, True, False))
    up_sec_disp[sec_msk_set_to_ref_alt] = 0
    up_sec_disp_msk[np.where(
        sec_ds[cst.EPI_MSK].values == 5, True, False)] = 255

    assert np.allclose(disp[cst.STEREO_SEC][cst.DISP_MAP].values,
                       up_sec_disp)
    assert np.allclose(disp[cst.STEREO_SEC][cst.DISP_MSK].values,
                       up_sec_disp_msk)
