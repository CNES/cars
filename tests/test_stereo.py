...
def test_compute_disparity_1():
    ...
    assert output['sec']['disp'].shape == (160, 177)
    assert output['sec']['msk'].shape == (160, 177)
    ...
    # Uncomment to update baseline
    #output['ref'].to_netcdf(absolute_data_path("ref_output/disp1_ref_pandora.nc"))
    #output['sec'].to_netcdf(absolute_data_path("ref_output/disp1_sec_pandora.nc"))
...
def test_compute_disparity_1_msk_sec():
    ...
    # Uncomment to update baseline
    #output['ref'].to_netcdf(absolute_data_path("ref_output/disp1_ref_pandora_msk_sec.nc"))
    #output['sec'].to_netcdf(absolute_data_path("ref_output/disp1_sec_pandora_msk_sec.nc"))
...
def test_create_inside_sec_roi_mask():
    ...
    ds_nb_col = 6
    ds_nb_row = 5
    ds = xr.Dataset({'im': (['row', 'col'], np.ones((ds_nb_row, ds_nb_col), dtype=bool))})
    ds.attrs['margins'] = np.array([ds_left_margin, ds_top_margin, ds_right_margin, ds_bottom_margin])
...
def test_estimate_color_from_disparity():
    ...
    disp_ds = xr.Dataset({'disp': (['row', 'col'], np.copy(disp)),
                          'msk': (['row', 'col'], np.copy(mask))},
                         coords={'row': disp_row, 'col': disp_col})

    disp_ds.attrs['roi'] = [margins[0], margins[1], disp_nb_col-margins[2], disp_nb_row-margins[3]]
    disp_ds.attrs['roi_with_margins'] = [0, 0, disp_nb_col, disp_nb_row]
    disp_ds.attrs['full_epipolar_size'] = [100, 100]
    ...
    clr_mask[4, 4] = 0
    clr_ds = xr.Dataset({'im': (['band', 'row', 'col'], np.copy(clr)),
                         'msk': (['row', 'col'], np.copy(clr_mask))},
                        coords={'band': range(clr_nb_band), 'row': clr_row, 'col': clr_col})

    # create fake secondary dataset
    sec_margins = [1, 1, 1, 1]
    sec_ds = xr.Dataset()
    sec_ds.attrs['margins'] = np.array(sec_margins)
    ...
    assert np.allclose(ref_mask, np.isnan(interp_clr_ds['im'].values[0,:,:]))

    ref_data = np.ceil(disp)
    ref_data[margins[1]:-margins[3], margins[0]:-margins[2]] += val
    ref_data[ref_mask] = 0

    interp_clr_msk = np.isnan(interp_clr_ds['im'].values[0, :, :])
    interp_clr_ds['im'].values[0, :, :][interp_clr_msk] = 0

    assert np.allclose(ref_data, interp_clr_ds['im'].values[0,:,:])
...
def test_geoid_offset():
    ...
    points = xr.Dataset({
        'x': geoid_ref.x, 'y': geoid_ref.y,
        'z': (('row', 'col'), np.zeros_like(geoid_ref.z))
    })
