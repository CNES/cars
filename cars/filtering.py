...
#   * on_ground_margin: margin added to the on ground region to not filter points clusters that were incomplete
#                       because they were on the edges
#   * pts_connection_dist: distance to use to consider that two points are connected
#   * nb_pts_threshold: points clusters that have less than this number of points will be filtered
#   * dist_between_clusters: distance to use to consider that two points clusters are far from each other or
#                            not. If a small points cluster is near to another one, it won't be filtered.
#                            (None = deactivated)
#   * construct_removed_elt_msk: if set to True, the removed points mask will be added to the cloud datasets
#                                in input of the simple_rasterization_dataset
#   * mask_value: value to use to identify the removed points in the mask
SmallComponentsFilterParams = namedtuple('SmallComponentsFilterParams', ['on_ground_margin',
                                                                         'connection_val',
                                                                         'nb_pts_threshold',
...
#                                in input of the simple_rasterization_dataset
#   * mask_value: value to use to identify the removed points in the mask
StatisticalFilterParams = namedtuple('StatisticalFilterParams', ['k',
                                                                 'std_dev_factor',
                                                                 'filtered_elt_msk',
                                                                 'msk_value'])
...
def small_components_filtering(cloud: pandas.DataFrame, connection_val: float, nb_pts_threshold: int,
                               clusters_distance_threshold: float=None, filtered_elt_pos: bool=False)\
    ...
    :param connection_val: distance to use to consider that two points are connected
    :param nb_pts_threshold: number of points to use to identify small clusters to filter
    :param clusters_distance_threshold: distance to use to consider if two points clusters are far from each other or
    not (set to None to deactivate this level of filtering)
    :param filtered_elt_pos: if filtered_elt_pos is set to True, the removed points positions in their original
...
def detect_small_components(cloud_xyz: np.ndarray, connection_val: float, nb_pts_threshold: int,
                            clusters_distance_threshold: float=None) -> List[int]:
    """
    Determine the indexes of the points of cloud_xyz to filter.
    The clusters are made of 'connected' points (2 connected points have a distance smaller than connection_val)
    The removed clusters are composed of less than nb_pts_threshold points and are also far from other clusters
    (points are further than clusters_distance_threshold). If clusters_distance_threshold is set to None, all the
    clusters that are composed of less than nb_pts_threshold points are filtered.

    :param cloud_xyz: points kdTree
    :param connection_val: distance to use to consider that two points are connected
    :param nb_pts_threshold: number of points to use to identify small clusters to filter
    :param clusters_distance_threshold: distance to use to consider if two points clusters are far from each other
    or not (set to None to deactivate this level of filtering)
    ...
    for idx in range(len(cloud_xyz)):
        ...
        # get point neighbors
        neighbors_list = cloud_tree.query_ball_point(cloud_xyz[idx], connection_val)
        ...
        # iteratively add all the neighbors of the points which were added to the current cluster (if there are some)
        while len(neighbors_list) != 0:
            all_neighbors = cloud_tree.query_ball_point(cloud_xyz[neighbors_list], connection_val)
    ...
    for idx in range(len(connected_components)):
        if len(connected_components[idx]) < nb_pts_threshold:
            if clusters_distance_threshold is not None:
                # search if the current cluster has any neighbors in the clusters_distance_threshold radius
                all_neighbors = cloud_tree.query_ball_point(cloud_xyz[connected_components[idx]],
                                                            clusters_distance_threshold)
                ...
                # retrieve only new neighbors
                neighbors_list = list(set(new_neighbors) - set(connected_components[idx]))
...
def statistical_outliers_filtering(cloud: pandas.DataFrame, k: int, std_factor: float, filtered_elt_pos: bool=False)\
        -> Tuple[pandas.DataFrame, Union[None, pandas.DataFrame]]:
    """
...
def detect_statistical_outliers(cloud_xyz: np.ndarray, k: int, std_factor: float=3.0) -> List[int] :
    """
    Determine the indexes of the points of cloud_xyz to filter.
    The removed points have mean distances with their k nearest neighbors that are greater
    than a distance threshold (dist_thresh).
    This threshold is computed from the mean (mean_distances) and standard deviation (stddev_distances) of all the
    points mean distances with their k nearest neighbors: dist_thresh = mean_distances + std_factor * stddev_distances
    ...
    # mean is not used directly as each line contained the distance value to the point itself
    mean_neighbors_distances = np.sum(neighbors_distances, axis=1)
    mean_neighbors_distances /= k


    # compute mean and standard deviation of those mean distances for the whole point cloud
    mean_distances = np.mean(mean_neighbors_distances)
    stddev_distances = np.std(mean_neighbors_distances)

    # compute distance threshold and apply it to determine which points will be removed
...
def filter_cloud(cloud:pandas.DataFrame, index_elt_to_remove: List[int], filtered_elt_pos:bool=False) \
        -> Tuple[pandas.DataFrame, Union[None, pandas.DataFrame]]:
    """
    Filter all points of the cloud DataFrame which index is in the index_elt_to_remove list.
    If filtered_elt_pos is set to True, the information of the removed elements positions in their original epipolar
    images are returned. To do so the cloud DataFrame has to be build with the 'with_coords' option activated.

    :param cloud: combined cloud as returned by the create_combined_cloud function
    :param index_elt_to_remove: indexes of lines to filter in the cloud DataFrame
    :param filtered_elt_pos: if filtered_elt_pos is set to True, the removed points positions in their original
    epipolar images are returned, otherwise it is set to None
    :return: Tuple composed of the filtered cloud DataFrame and the filtered elements epipolar position information
    (or None for the latter if filtered_elt_pos is set to False or if the cloud Dataframe has not been build with the
    with_coords option)
    """
...
def add_cloud_filtering_msk(clouds_list: List[xr.Dataset], elt_pos_infos: pandas.DataFrame, mask_label: str,
                            mask_value: int=255):
    """
    ...
    if elt_pos_infos is None or cst.POINTS_CLOUD_COORD_EPI_GEOM_I not in elt_pos_infos.columns or \
        ...
        if min_elt_index < 0 or max_elt_index > len(clouds_list) - 1:
            raise Exception('Index indicated in the elt_pos_infos pandas.DataFrame is not coherent '
