...
+------------------------------+---------------------------------------------------------------------+-----------------------------+----------------------+----------+
| *ignore_roi_during_a_priori* | Ignore the roi during a priori computation                          | bool                        | false                | No       |
+------------------------------+---------------------------------------------------------------------+-----------------------------+----------------------+----------+
...
.. tabs::
        ...
        **Advanced configuration**
        ...
        .. tabs::
            ...
            .. tab:: Edges
                ...
                | Name           | Description                                                                                         | Type   | Default value | Required |
                +================+=====================================================================================================+========+===============+==========+
                | *edges_mask*   | File path. A mask specifying for each pixel whether it is an edge (1) or not (0).                   | str    | None          | No       |
                +----------------+-----------------------------------------------------------------------------------------------------+--------+---------------+----------+
                | *depth_map*    | File path. A depth map, specifying the estimated distance of each pixel from the camera.            | str    | None          | No       |
                +----------------+-----------------------------------------------------------------------------------------------------+--------+---------------+----------+
                | *normals*      | File path. A normal map, specifying the estimated surface normals for each pixel.                   | str    | None          | No       |
                +----------------+-----------------------------------------------------------------------------------------------------+--------+---------------+----------+
                | *tile_id*      | File path. A tile ID map, specifying the tile ID of each pixel, resulting from parallel processing. | str    | None          | No       |
                +----------------+-----------------------------------------------------------------------------------------------------+--------+---------------+----------+
    ...
    .. tab:: Filling
        ...
        +----------------------------+---------------------------------------------------------------------------------------------+-----------+--------------------------+----------+
        | *fill_with_endogenous_dem* | Value(s) for which pixels will be filled with a DEM generated by CARS (rivers, occlusion)   | int, list | None                     | No       |
        +----------------------------+---------------------------------------------------------------------------------------------+-----------+--------------------------+----------+
        | *fill_with_exogenous_dem*  | Value(s) for which pixels will be filled with the DEM given by the user (cloud)             | int, list | None                     | No       |
        +----------------------------+---------------------------------------------------------------------------------------------+-----------+--------------------------+----------+
        | *interpolation*            | Value(s) for which pixels will be filled by interpolating neighboring values (mismatch)     | int, list | None                     | No       |
        +----------------------------+---------------------------------------------------------------------------------------------+-----------+--------------------------+----------+
